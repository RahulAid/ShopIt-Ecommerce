...
import { getAllCategories,fetchAsyncProductsOfCategory, removeCategoryProducts} from '../../store/categorySlice';
    ...
    <nav className='navbar'>
      <div className='navbar-cnt flex align-center'>
        <div className='brand-and-toggler flex align-center'>
          ...
          <Link onClick={()=>dispatch(removeCategoryProducts())}/* to = "/" */ className='navbar-brand flex align-center'>
          ...
          <ul className='navbar-nav flex align-center fs-12 fw-4 font-manrope'>
            {
              ...
              categories.slice(0, 8).map((category, idx) => (
                <li className='nav-item no-wrap' key = {idx}>
                  {/* <Link to = {`category/${category?.slug}`} className='nav-link text-capitalize'>{category?.slug.replace("-", " ")}</Link> */}
                  <Link className='nav-link text-capitalize' onClick={()=>dispatch(fetchAsyncProductsOfCategory(category?.slug))}>{category?.slug.replace("-", " ")}</Link>
                </li>
              ))
            }
          </ul>
